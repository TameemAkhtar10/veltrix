...
//#endregion
//#region node_modules/markdown/lib/markdown.js
var require_markdown = /* @__PURE__ */ __commonJS({ "node_modules/markdown/lib/markdown.js": ((exports) => {
	(function(expose) {
		/**
		*  class Markdown
		*
		...
		**/
		Markdown.dialects.Gruber = { block: {
			...
			horizRule: function horizRule(block, next) {
				...
				if (!m) return;
				var jsonml = [["hr"]];
				if (m[1]) jsonml.unshift.apply(jsonml, this.processBlock(m[1], []));
				if (m[3]) next.unshift(mk_block(m[3]));
				return jsonml;
			},
			lists: (function() {
				var any_list = "[*+-]|\\d+\\.", bullet_list = /[*+-]/, is_list_re = /* @__PURE__ */ new RegExp("^( {0,3})(" + any_list + ")[ 	]+"), indent_re = "(?: {0,3}\\t| {4})";
				function regex_for_depth(depth) {
					return /* @__PURE__ */ new RegExp("(?:^(" + indent_re + "{0," + depth + "} {0,3})(" + any_list + ")\\s+)|(^" + indent_re + "{0," + (depth - 1) + "}[ ]{0,4})");
				}
				...
				function add(li, loose, inline, nl) {
					...
					for (var i = 0; i < inline.length; i++) {
						var what = inline[i];
						if (typeof what == "string" && add_to.length > 1 && typeof add_to[add_to.length - 1] == "string") add_to[add_to.length - 1] += what;
						else add_to.push(what);
					}
				}
				function get_contained_blocks(depth, blocks) {
					var re = /* @__PURE__ */ new RegExp("^(" + indent_re + "{" + depth + "}.*?\\n?)*$"), replace = new RegExp("^" + indent_re + "{" + depth + "}", "gm"), ret = [];
					while (blocks.length > 0) if (re.exec(blocks[0])) {
				...
				return function(block, next) {
					...
					if (!m) return void 0;
					function make_list(m$1) {
						var list$1 = bullet_list.exec(m$1[2]) ? ["bulletlist"] : ["numberlist"];
						stack.push({
							list: list$1,
							indent: m$1[1]
						});
			...
			referenceDefn: function referenceDefn(block, next) {
				...
				if (attrs.references === void 0) attrs.references = {};
		...
		Markdown.dialects.Gruber.inline = {
			__oneElement__: function oneElement(text, patterns_or_re, previous_nodes) {
				...
				m = (/* @__PURE__ */ new RegExp("([\\s\\S]*?)(" + (patterns_or_re.source || patterns_or_re) + ")")).exec(text);
				if (!m) return [text.length, text];
				else if (m[1]) return [m[1].length, m[1]];
				var res;
				if (m[2] in this.dialect.inline) res = this.dialect.inline[m[2]].call(this, text.substr(m.index), m, previous_nodes || []);
			...
			"![": function image(text) {
				...
				m = text.match(/^!\[(.*?)\][ \t]*\[(.*?)\]/);
				if (m) return [m[0].length, ["img_ref", {
					alt: m[1],
					ref: m[2].toLowerCase(),
					original: m[0]
				}]];
			...
			"[": function link(text) {
				...
				if (!res) return [1, "["];
				var consumed = 1 + res[0], children = res[1], link$1, attrs;
				...
				if (m) {
					...
					if (!m[3]) {
						var open_parens = 1;
						for (var len = 0; len < url.length; len++) switch (url[len]) {
							case "(":
								open_parens++;
								break;
							case ")":
					...
					link$1 = ["link", attrs].concat(children);
					return [consumed, link$1];
				}
				m = text.match(/^\s*\[(.*?)\]/);
				if (m) {
					consumed += m[0].length;
					...
					link$1 = ["link_ref", attrs].concat(children);
					return [consumed, link$1];
				}
			...
			"<": function autoLink(text) {
				var m;
				if ((m = text.match(/^<(?:((https?|ftp|mailto):[^>]+)|(.*?@.*?\.[a-zA-Z]+))>/)) != null) if (m[3]) return [m[0].length, [
					"link",
					{ href: "mailto:" + m[3] },
					m[3]
				]];
				else if (m[2] == "mailto") return [m[0].length, [
					"link",
					{ href: m[1] },
					m[1].substr(7)
				]];
				else return [m[0].length, [
					"link",
					{ href: m[1] },
					m[1]
				]];
			...
			return function(text, orig_match) {
				if (this[state_slot][0] == md) {
					...
					if (last instanceof CloseTag) {
						res.pop();
						return [text.length - last.len_after, [tag].concat(res)];
					} else {
		...
		Markdown.buildInlinePatterns = function(d) {
			...
			d.__call__ = function(text, pattern) {
				if (pattern != void 0) return fn.call(this, text, pattern);
		...
		Markdown.dialects.Maruku.processMetaHash = function processMetaHash(meta_string) {
			...
			for (var i = 0; i < meta.length; ++i) if (/^#/.test(meta[i])) attr.id = meta[i].substring(1);
		...
		Markdown.dialects.Maruku.block.table = function table(block, next) {
			var _split_on_unescaped = function(s, ch) {
				ch = ch || "\\s";
				if (ch.match(/^[\\|\[\]{}?*.+^$]$/)) ch = "\\" + ch;
				var res = [], r = /* @__PURE__ */ new RegExp("^((?:\\\\.|[^\\\\" + ch + "])*)" + ch + "(.*)"), m$1;
				while (m$1 = s.match(r)) {
					res.push(m$1[1]);
					s = m$1[2];
				}
			...
			if (m = block.match(leading_pipe)) m[3] = m[3].replace(/^\s*\|/gm, "");
			else if (!(m = block.match(no_leading_pipe))) return;
			var table$1 = [
				"table",
				["thead", ["tr"]],
				["tbody"]
			...
			for (i = 0; i < m[1].length; i++) table$1[1][1].push(["th", html_attrs[i] || {}].concat(this.processInline(m[1][i].trim())));
		...
		var forEach;
		if (Array.prototype.forEach) forEach = function(arr, cb, thisp) {
			return arr.forEach(cb, thisp);
		};
		else forEach = function(arr, cb, thisp) {
		...
		*/
		...
		function render_tree(jsonml) {
			...
			while (jsonml.length) content.push(render_tree(jsonml.shift()));
			var tag_attrs = "";
			for (var a$1 in attributes) tag_attrs += " " + a$1 + "=\"" + escapeHTML(attributes[a$1]) + "\"";
		...
		function convert_tree_to_html(tree, references, options) {
			...
			if (attrs) {
				jsonml[1] = {};
				for (i in attrs) jsonml[1][i] = attrs[i];
				attrs = jsonml[1];
			}
		...
		function merge_text_nodes(jsonml) {
			...
			while (i < jsonml.length) if (typeof jsonml[i] === "string") if (i + 1 < jsonml.length && typeof jsonml[i + 1] === "string") jsonml[i] += jsonml.splice(i + 1, 1)[0];
			else ++i;
			else {
...
//#endregion
//#region node_modules/markdown/lib/index.js
var require_lib = /* @__PURE__ */ __commonJS({ "node_modules/markdown/lib/index.js": ((exports) => {
